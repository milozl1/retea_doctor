...
export function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Sync user to retea_doctor network_users (fire-and-forget, non-fatal)
    try {
  ...
  const handleGoogleSignIn = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${origin}/auth/callback?redirectTo=${encodeURIComponent(
          redirectTo.startsWith("/") ? redirectTo : "/"
        )}`,
      },
    });
    if (error) toast.error("Eroare la autentificare cu Google");
  };
